...
export async function renderInTestDom(element: ReactNode): Promise<TestDom> {
  ...
  if (windowObject.location === undefined) {
  ...
  const dispatch = async (target: Element, type: string, init: EventInit = {}): Promise<void> => {
    await act(async () => {
      target.dispatchEvent(
        new eventConstructor(type, { bubbles: true, cancelable: true, ...init }),
      );
      await Promise.resolve();
    });
  };
