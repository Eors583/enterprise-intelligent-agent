...
import {
  TOOL_RECONCILIATION_EVENT_TYPE,
...
interface ClaimedRow {
  ...
  readonly event_type: typeof TOOL_RECONCILIATION_EVENT_TYPE;
...
@Injectable()
export class PrismaToolReconciliationQueueRepository extends ToolReconciliationQueueRepository {
    ...
    return this.withWorkerRole(async (transaction) => {
      const rows = await transaction.$queryRaw<ClaimedRow[]>(Prisma.sql`
        WITH candidates AS MATERIALIZED (
          ...
          WHERE delivery."consumer_key" = ${OUTBOX_CONSUMER.toolReconciliation}
            AND delivery."lane" = ${OUTBOX_LANE.toolReconciliation}
            AND event."event_type" = ${TOOL_RECONCILIATION_EVENT_TYPE}
  ...
  markProcessed(input: {
    ...
    return this.transition(
      ...
      Prisma.sql`
        ...
        "provider_name" = 'tool-gateway-reconciler',
  ...
  markFailed(input: {
    ...
    return this.transition(
      ...
      Prisma.sql`
        ...
        "last_error" = ${`${safeCode(input.reasonCode)}: Tool reconciliation event rejected.`}
      `,
    );
  }
    ...
    return this.withWorkerRole(async (transaction) => {
      const updated = await transaction.$executeRaw(Prisma.sql`
        ...
        WHERE "event_id" = ${eventId}::uuid
          AND "consumer_key" = ${OUTBOX_CONSUMER.toolReconciliation}
  ...
  private withWorkerRole<T>(
    ...
    if (!this.prisma.enabled) {
      throw new Error('The Tool reconciliation worker requires the Prisma adapter.');
    }
