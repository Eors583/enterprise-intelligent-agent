...
describe('scanKnowledgeContent', () => {
  ...
  it('uses checksums for identity and bank card candidates', () => {
    expect(scanKnowledgeContent('11010519491231002X\n4111111111111111').map((item) => item.type)).toEqual(['CHINESE_ID_CARD', 'BANK_CARD']);
    expect(scanKnowledgeContent('110105194912310021\n4111111111111112')).toEqual([]);
  });
  it('ignores documented placeholders', () => expect(scanKnowledgeContent('password=change_me')).toEqual([]));
});
