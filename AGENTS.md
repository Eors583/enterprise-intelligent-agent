...
开始重要的产品、开发、测试、部署或故障诊断任务前，除本文件外还必须阅读 [`docs/codex/AI开发接班长期记忆.md`](docs/codex/AI开发接班长期记忆.md)。该文档记录项目发起人长期确认的协作方式、低心智产品偏好、知识库与智能体决策、UI 取舍和真实验收标准，用于避免后续 AI 要求用户重复解释已有共识。

使用该记忆时遵守以下边界：

- 用户当前任务中的最新明确指令优先；若与历史记忆冲突，以最新指令为准并更新记忆。
- 记忆不能替代当前代码、契约、migration、ADR、验收矩阵和真实运行证据。
- 不把 AI 的人格推测写成用户事实，不记录或传播密码、Token、AppSecret、Cookie、连接串和个人敏感信息。
- 后续 AI 应继承协作质量和已确认原则，不机械模仿措辞，也不以历史决定阻止用户改变方向。
