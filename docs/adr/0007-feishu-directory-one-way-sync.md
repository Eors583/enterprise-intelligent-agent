...
| `FEISHU_DIRECTORY_INITIAL_PASSWORD`   | `1234567890`             | 飞书新员工或尚无密码凭据的飞书员工使用的临时密码，登录后必须立即修改；已有密码永不覆盖       |
...
新导入用户固定为 `MEMBER`，并创建标记为 `mustChangePassword=true` 的临时 `PasswordCredential`。已有飞书绑定但仍无密码凭据的成员会在下一次同步时补齐临时密码；已有密码绝不覆盖。无论飞书返回什么资料，同步都不能提升角色或自动接管本地特权账号，`LOCKED` 状态也不会被同步解锁。临时密码账号在完成修改密码前只允许访问认证必要接口。
...
- 本地邮箱冲突不合并账户，新用户创建 `MEMBER` 临时密码凭据并强制首次修改，已有密码不被覆盖，不能提升或停用特权角色；
